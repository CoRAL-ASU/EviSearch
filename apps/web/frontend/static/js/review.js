...
(function () {
    ...
    // The page reviews one extraction; which one is not the reviewer's problem, so it is never named or chosen here.
    // A ?run= link (from the table grid, or an older bookmark) still opens exactly that extraction.
    async function start() {
        const t = await get(`/api/tables/${enc(TABLE)}`);
        if (!t.success) return toast(t.error, 'error');
        runs = t.runs.slice().reverse();
        if (!runs.length) { say('Nothing to review yet — extract some papers first.'); return; }
        if (!runs.some((r) => r.run === run)) run = t.showcase_run || runs[0].run;
        await loadQueue();
    }
    function say(text, cls) {
        $('cell-top').innerHTML = `<div class="card bg-base-100 shadow"><div class="card-body p-6 ${cls || 'opacity-60'}">${esc(text)}</div></div>`;
        $('cell-review').innerHTML = '';
    }
    ...
    function unsavedOk() {
        ...
        const ta = document.querySelector('#cell-review .val');
        if (!ta) return true;
    ...
    async function select(it) {
        ...
        if (!cellsCache.has(it.doc_id)) {
            say('Loading the paper’s cells…');
            const r = await get(`/api/runs/${enc(run)}/docs/${enc(it.doc_id)}/cells`);
            if (!r.success) { say(r.error, 'text-error'); return; }
            cellsCache.set(it.doc_id, r);
        }
        renderCell();
        window.scrollTo({top: 0, behavior: 'smooth'});  // the column is long now; start at the definition
    }
    ...
    function renderCell() {
        ...
        const cell = cellOf(current.doc_id, current.column);
        if (!cell) { say('This column is not in the extracted output.'); return; }
        ...
        // above the paper: what the column means and what each arm answered
        $('cell-top').innerHTML = `<div class="card bg-base-100 shadow"><div class="card-body p-4 space-y-3">
        ...
        // below the paper: the reviewer's own answer, so it is filled in after reading the page
        $('cell-review').innerHTML = `<div class="card bg-base-100 shadow"><div class="card-body p-4 space-y-2">
            <div class="flex items-center gap-2"><span class="text-sm font-semibold">Your review</span>
                <span class="text-xs opacity-60">${esc(cell.column)} · ${esc(shortDoc(data.name))}</span>
        ...
        document.querySelector('#cell-review .save').onclick = () => {
            ...
            save(ta.value, reason, document.querySelector('#cell-review .note').value);
        };
        document.querySelector('#cell-review .nr').onclick = () => {
            const reason = document.querySelector('#cell-review .why').value || 'should be Not reported';
            save('Not reported', reason, document.querySelector('#cell-review .note').value);
        };
        document.querySelector('#cell-review .rule').onclick = () => openRule(cell);
        const undo = document.querySelector('#cell-review .undo');
