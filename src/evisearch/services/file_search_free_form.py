...
RESULTS_ROOT = PROJECT_ROOT / "experiment-scripts" / "baselines_file_search_results" / "free_form"
# ─── Helper Functions ──────────────────────────────────────────────────────────
...
def _resolve_pdf_path(pdf_arg: str) -> Path:
    ...
    if not pdf_path.is_absolute() and not pdf_path.exists():
        resolved = PROJECT_ROOT / pdf_arg
        if resolved.exists():
            pdf_path = resolved
    if not pdf_path.exists():
        raise FileNotFoundError(
            f"PDF not found: {pdf_arg}\n"
            f"Resolved path: {pdf_path.resolve()}\n"
            "Run from repo root or use absolute path."
        )
    if pdf_path.stat().st_size == 0:
        raise ValueError(f"PDF is empty: {pdf_arg}")
    return pdf_path
...
def run_file_search_free_form_baseline(argv: List[str] | None = None) -> None:
    ...
    try:
        ...
        if args.reliability_runs > 1:
            ...
            _, total_in, total_out = extract_once(
                provider=provider,
                label_groups=label_groups,
                definitions=definitions,
                structurer=structurer,
