...
"""
baseline_landing_ai_w_gpt4.py

GPT-4.1 baseline using Landing-AI parsed markdown as the document context.
Same flow as baseline_landing_ai_w_gemini but uses OpenAI GPT-4.1 instead.
...
Output format mirrors baseline_landing_ai_w_gemini:
...
RESULTS_ROOT = (
    repo_root / "experiment-scripts" / "baseline_landing_ai_w_gpt4" / "results"
)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        "Landing-AI markdown baseline with GPT-4.1 (native JSON)"
    )
    parser.add_argument(
        "--trial",
        required=True,
    ...
    print(f"\n{'=' * 70}")
    print("BASELINE: Landing-AI parsed markdown + GPT-4.1 (native JSON)")
    ...
    provider = GPT4MarkdownProvider(args.model)
    ...
    cost_metrics = {
        "provider": "openai",
        "method": "baseline_landing_ai_w_gpt4",
        "model": args.model,
        "trial": trial_name,
