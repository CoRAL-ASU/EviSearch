...
def test_the_knowledge_page_shows_the_notes_who_reads_them_and_the_runs_on_them(ws, tmp_path, monkeypatch):
    ...
    (kb / "retired").mkdir(parents=True)
    (kb / "retired" / "conventions.jsonl").write_text(json.dumps({"at": "t", "op": "create", "id": "cv-0001", "by": "seed", "record": {
        "id": "cv-0001", "status": "approved", "instruction": "- old region rule", "trigger": {"scope": "family"}, "source": {"kind": "seed"}}}) + "\n")
    fp = notes.fingerprint()
    root = runtime_paths.RESULTS_ROOT
    _write(root / DOC / "runs" / "r1" / "agent_extractor" / "extraction_metadata.json", {"extraction_rules": f"notes:{fp}"})
    _write(root / DOC / "runs" / "r2" / "agent_extractor" / "extraction_metadata.json", {"extraction_rules": "notes:0123456789ab:scoped"})
    ...
    assert region["supersedes"] == [{"id": "cv-0001", "instruction": "- old region rule", "status": "approved", "source": "seed"}]
    ...
    assert runs["r1"]["current"] and runs["r1"]["delivery"] == "all"
    assert not runs["r2"]["current"] and runs["r2"]["delivery"] == "scoped" and runs["r2"]["fingerprint"] == "0123456789ab"
    assert r["retired"] == [{"id": "cv-0001", "instruction": "- old region rule", "status": "approved", "scope": "family",
                             "source": "seed", "replaced_by": ["regions"]}]
    ...
    assert sorted(readers["Markdown baseline"]) == ["figures", "medians", "regions"]  # one pass over the whole table
